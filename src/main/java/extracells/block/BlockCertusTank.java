	...
	public boolean onBlockActivated(World worldObj, int x, int y, int z, EntityPlayer player, int blockID, float offsetX, float offsetY, float offsetZ)
	{
		...
		if (player.isSneaking() && current != null)
		{
			try
			{
				...
				{
					this.dropBlockAsItem(worldObj, x, y, z, this.getDropWithNBT(worldObj, x, y, z));
			...
			if (liquid != null)
			{
				...
				if (amountFilled != 0 && !player.capabilities.isCreativeMode)
					if (current.stackSize > 1)
					{
						...
						player.inventory.addItemStackToInventory(current.getItem().getContainerItem(current));
					}
					else

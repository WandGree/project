		...
		implements IActionHost, ICraftingProvider, ICraftingWatcherHost, IECTileEntity
{
	...
	private class FluidCrafterInventory implements IInventory
	{
		...
		public boolean isItemValidForSlot(int slot, ItemStack stack)
		{
			...
			{
				ICraftingPatternDetails details = ((ICraftingPatternItem) stack.getItem()).getPatternForItem(stack, TileEntityFluidCrafter.this.getWorldObj());
	...
	public void provideCrafting(ICraftingProviderHelper craftingTracker)
	{
		...
		ICraftingPatternDetails[] oldHandler = this.patternHandlerSlot;
		this.patternHandlerSlot = new ICraftingPatternDetails[9];
		for (int i = 0; this.inventory.inv.length > i; i++)
		{
			...
			if (currentPatternStack != null && oldItem != null && ItemStack.areItemStacksEqual(currentPatternStack, oldItem))
			{
				...
				if (pa != null)
				{
					this.patternHandlerSlot[i] = pa;
					this.patternHandlers.add(pa);
			...
			{
				Item item = currentPatternStack.getItem();
				if (item instanceof ICraftingPatternItem)
				{
					ICraftingPatternItem currentPattern = (ICraftingPatternItem) item;

					/* TODO gamerforEA code replace, old code:
					if (currentPattern.getPatternForItem(currentPatternStack, this.getWorldObj()) != null && currentPattern.getPatternForItem(currentPatternStack, this.getWorldObj()).isCraftable())
					{
						ICraftingPatternDetails pattern = new CraftingPattern(currentPattern.getPatternForItem(currentPatternStack, this.getWorldObj()));
					...
					ICraftingPatternDetails patternForItem = currentPattern.getPatternForItem(currentPatternStack, this.getWorldObj());
					if (patternForItem != null && patternForItem.isCraftable())
					{
						ICraftingPatternDetails pattern = new CraftingPattern(patternForItem);
						this.patternHandlers.add(pattern);
						this.patternHandlerSlot[i] = pattern;
						if (pattern.getCondensedInputs().length == 0)
							craftingTracker.setEmitable(pattern.getCondensedOutputs()[0]);
			...
			this.oldStack[i] = currentPatternStack;
		}
		this.updateWatcher();
	}
		...
		if (patternDetails instanceof CraftingPattern)
		{
			...
			for (Fluid fluid : fluids.keySet())
			{
				...
				if (extractFluid == null)
					return false;
				if (extractFluid.getStackSize() < amount)
				{
					storage.getFluidInventory().injectItems(extractFluid, Actionable.MODULATE, new MachineSource(this));
					return false;
				}
				// TODO gamerforEA code end
			}
			this.finishCraftingTime = System.currentTimeMillis() + 1000;
			this.returnStack = patter.getOutput(table, this.getWorldObj());
	...
	{
		if (this.getWorldObj() == null || this.getWorldObj().provider == null)
			return;
		if (this.update)
		{
			this.update = false;
			if (this.getGridNode() != null && this.getGridNode().getGrid() != null)
				this.getGridNode().getGrid().postEvent(new MENetworkCraftingPatternChange(this.instance, this.getGridNode()));
		}
		if (this.isBusy && this.finishCraftingTime <= System.currentTimeMillis() && this.getWorldObj() != null && !this.getWorldObj().isRemote)
		{
		...
		if (!this.isBusy && this.getWorldObj() != null && !this.getWorldObj().isRemote)
		{
	...
	private void updateWatcher()
	{
		...
		for (ICraftingPatternDetails patter : this.patternHandlers)
		{
			...
			if (patter.getCondensedInputs().length == 0)
			{
				...
				if (crafting != null && crafting.isRequesting(patter.getCondensedOutputs()[0]))

...
public class TileEntityFluidInterface extends TileBase
		implements IActionHost, IFluidHandler, IECTileEntity, IFluidInterface, IFluidSlotPartOrBlock,
		ITileStorageMonitorable, IStorageMonitorable, ICraftingProvider, IWailaTile
{
	...
	private class FluidInterfaceInventory implements IInventory
	{
		...
		public boolean isItemValidForSlot(int slot, ItemStack stack)
		{
			...
			{
				ICraftingPatternDetails details = ((ICraftingPatternItem) stack.getItem()).getPatternForItem(stack, TileEntityFluidInterface.this.getWorldObj());
				return details != null;
			}
			return false;
		}
		...
		public boolean isUseableByPlayer(EntityPlayer player)
		{
			...
			// return true;
			return ModUtils.isUseableByPlayer(TileEntityFluidInterface.this, player);
			// TODO gamerforEA code end
		}
	...
	public int fill(ForgeDirection from, FluidStack resource, boolean doFill)
	{
		...
		if ((this.tanks[from.ordinal()].getFluid() == null || this.tanks[from.ordinal()].getFluid().getFluid() == resource.getFluid()) && resource.getFluid() == FluidRegistry.getFluid(this.fluidFilter[from.ordinal()]))
		{
		...
		int filled = 0;
		filled += this.fillToNetwork(resource, doFill);
	...
	{
		this.getWorldObj().markBlockForUpdate(this.yCoord, this.yCoord, this.zCoord);
	...
	{
		if (FMLCommonHandler.instance().getSide().isClient() && (this.getWorldObj() == null || this.getWorldObj().isRemote))
			return null;
	...
	public void provideCrafting(ICraftingProviderHelper craftingTracker)
	{
		...
		// TODO gamerforEA code start
		if (this.lastInvModCount != this.inventory.modCount)
		{
			Arrays.fill(this.cachedPatterns, null);
			for (int i = 0; i < this.cachedPatterns.length; i++)
			{
				ItemStack patternStack = this.inventory.inv[i];
				if (patternStack != null)
				{
					Item patternItem = patternStack.getItem();
					if (patternItem instanceof ICraftingPatternItem)
					{
						ICraftingPatternItem craftingPatternItem = (ICraftingPatternItem) patternItem;
						this.cachedPatterns[i] = craftingPatternItem.getPatternForItem(patternStack, this.getWorldObj());
					}
				}
			}
			this.lastInvModCount = this.inventory.modCount;
		}
		// TODO gamerforEA code end
		...
		{
			/* TODO gamerforEA code replace, old code:
			ItemStack patternStack = inv[i];
			if (patternStack != null && patternStack.getItem() != null && patternStack.getItem() instanceof ICraftingPatternItem)
			{
				ICraftingPatternItem currentPattern = (ICraftingPatternItem) patternStack.getItem();

				if (currentPattern != null && currentPattern.getPatternForItem(patternStack, this.getWorldObj()) != null)
				{
					IFluidCraftingPatternDetails pattern = new CraftingPattern2(currentPattern.getPatternForItem(patternStack, this
							.getWorldObj()));
					this.patternHandlers.add(pattern);
					ItemStack is = this.makeCraftingPatternItem(pattern);
					if (is == null)
						continue;
					ICraftingPatternDetails p = ((ICraftingPatternItem) is.getItem()).getPatternForItem(is, this.getWorldObj());
					this.patternConvert.put(p, pattern);
					craftingTracker.addCraftingOption(this, p);
				}
			} */
			ICraftingPatternDetails patternForItem = this.cachedPatterns[i];
			if (patternForItem != null)
			{
				IFluidCraftingPatternDetails pattern = new CraftingPattern2(patternForItem);
				this.patternHandlers.add(pattern);
				ItemStack is = this.makeCraftingPatternItem(pattern);
				if (is != null)
				{
	...
	{
		this.export.removeAll(this.removeFromExport);
		this.removeFromExport.clear();
		this.export.addAll(this.addToExport);
		this.addToExport.clear();
		if (!this.hasWorldObj() || this.export.isEmpty())
			return;
		ForgeDirection[] directions = ForgeDirection.VALID_DIRECTIONS;
		for (ForgeDirection dir : directions)
		{
			TileEntity tile = this.getWorldObj().getTileEntity(this.xCoord + dir.offsetX, this.yCoord + dir.offsetY, this.zCoord + dir.offsetZ);
			if (tile != null)
			{
	...
	public boolean pushPattern(ICraftingPatternDetails patDetails, InventoryCrafting table)
	{
		...
		if (patternDetails instanceof CraftingPattern)
		{
			...
			for (Fluid fluid : fluids.keySet())
			{
				...
				IAEFluidStack extractFluid = storage.getFluidInventory().extractItems(AEApi.instance().storage().createFluidStack(new FluidStack(fluid, amount.intValue())), Actionable.SIMULATE, new MachineSource(this));
			...
			{
				Long amount = fluids.get(fluid);
				IAEFluidStack extractFluid = storage.getFluidInventory().extractItems(AEApi.instance().storage().createFluidStack(new FluidStack(fluid, amount.intValue())), Actionable.MODULATE, new MachineSource(this));
	...
	public void readFromNBT(NBTTagCompound tag)
	{
		...
		if (tag.hasKey("export"))
			this.readOutputFromNBT(tag.getCompoundTag("export"));
	}
	...
	{
		this.setFilter(ForgeDirection.getOrientation(_index), _fluid);
	}
	...
	private void tick()
	{
		...
		for (int i = 0; i < this.tanks.length; i++)
		{
			if (this.tanks[i].getFluid() != null && FluidRegistry.getFluid(this.fluidFilter[i]) != this.tanks[i].getFluid().getFluid())
			{
				...
				if (s != null)
				{
					...
					if (notAdded != null)
					{
						...
						storage.getFluidInventory().injectItems(AEApi.instance().storage().createFluidStack(this.tanks[i].drain(toAdd, true)), Actionable.MODULATE, new MachineSource(this));
						this.doNextUpdate = true;
						this.wasIdle = false;
					}
					else
					{
						storage.getFluidInventory().injectItems(AEApi.instance().storage().createFluidStack(this.tanks[i].drain(s.amount, true)), Actionable.MODULATE, new MachineSource(this));
						this.doNextUpdate = true;
						this.wasIdle = false;
					}
				}
			}
			if ((this.tanks[i].getFluid() == null || this.tanks[i].getFluid().getFluid() == FluidRegistry.getFluid(this.fluidFilter[i])) && FluidRegistry.getFluid(this.fluidFilter[i]) != null)
			{
				IAEFluidStack extracted = storage.getFluidInventory().extractItems(AEApi.instance().storage().createFluidStack(new FluidStack(FluidRegistry.getFluid(this.fluidFilter[i]), 125)), Actionable.SIMULATE, new MachineSource(this));
				if (extracted == null)
					continue;
				int accepted = this.tanks[i].fill(extracted.getFluidStack(), false);
				if (accepted == 0)
					continue;
				this.tanks[i].fill(storage.getFluidInventory().extractItems(AEApi.instance().storage().createFluidStack(new FluidStack(FluidRegistry.getFluid(this.fluidFilter[i]), accepted)), Actionable.MODULATE, new MachineSource(this)).getFluidStack(), true);
				this.doNextUpdate = true;
				this.wasIdle = false;
			}
		}
	...
	{
		if (this.getWorldObj() == null || this.getWorldObj().provider == null || this.getWorldObj().isRemote)
			return;
		if (this.update)
		{
			this.update = false;
			...
			{
				this.getGridNode(ForgeDirection.UNKNOWN).getGrid().postEvent(new MENetworkCraftingPatternChange(this, this.getGridNode(ForgeDirection.UNKNOWN)));
			}
		}
		this.pushItems();

...
public class TileEntityFluidFiller extends TileBase
		implements IActionHost, ICraftingProvider, IECTileEntity, IMEMonitorHandlerReceiver<IAEFluidStack>,
		IListenerTile
{
	...
	public void cellUpdate(MENetworkCellArrayUpdate event)
	{
		...
		if (storage != null)
			this.postChange(storage.getFluidInventory(), null, null);
	}
	...
	private ItemStack getPattern(ItemStack emptyContainer, ItemStack filledContainer)
	{
		...
		Preconditions.checkNotNull(this.encodedPattern, "encodedPattern must not be null");
	...
	public void provideCrafting(ICraftingProviderHelper craftingTracker)
	{
		...
		for (IAEFluidStack fluidStack : fluidStorage.getStorageList())
		{
			...
			if (maxCapacity == 0)
				continue;
			MutablePair<Integer, ItemStack> filled = FluidUtil.fillStack(this.containerItem.copy(), new FluidStack(fluid, maxCapacity));
			if (filled.right == null)
				continue;
			ItemStack pattern = this.getPattern(this.containerItem, filled.right);
			ICraftingPatternItem patter = (ICraftingPatternItem) pattern.getItem();
			ICraftingPatternDetails patternForItem = patter.getPatternForItem(pattern, this.getWorldObj());
			...
			craftingTracker.addCraftingOption(this, patternForItem);
		}
	...
	public void readFromNBT(NBTTagCompound tagCompound)
	{
		...
		if (tagCompound.hasKey("container"))
			this.containerItem = ItemStack.loadItemStackFromNBT(tagCompound.getCompoundTag("container"));
		else if (tagCompound.hasKey("isContainerEmpty") && tagCompound.getBoolean("isContainerEmpty"))
			this.containerItem = null;
		if (tagCompound.hasKey("return"))
			this.returnStack = ItemStack.loadItemStackFromNBT(tagCompound.getCompoundTag("return"));
	...
	public void securityBreak()
	{
		if (this.getWorldObj() != null)
			this.getWorldObj().func_147480_a(this.xCoord, this.yCoord, this.zCoord, true);
	}

		...
		implements IMEMonitorHandlerReceiver<IAEFluidStack>, IFluidSelectorContainer
{
	...
	public ContainerGasTerminal(PartGasTerminal _terminal, EntityPlayer _player)
	{
		...
		this.addSlotToContainer(new SlotRespective(this.terminal.getInventory(), 0, 8, 92));
		// Input Slot accepts nothing
		this.addSlotToContainer(new SlotFurnace(this.player, this.terminal.getInventory(), 1, 26, 92));
	...
	protected void bindPlayerInventory(InventoryPlayer inventoryPlayer)
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				this.addSlotToContainer(new Slot(inventoryPlayer, j + i * 9 + 9, 8 + j * 18, i * 18 + 122));
			}
		}
	...
	public ItemStack slotClick(int slotNumber, int p_75144_2_, int p_75144_3_, EntityPlayer player)
	{
		...
		if (slotNumber == 0 || slotNumber == 1)
		{
			...
			else if (GasUtil.isEmpty(stack) && PermissionUtil.hasPermission(player, SecurityPermissions.INJECT, (IPart) this.getTerminal()))
			{
			}
			else if (GasUtil.isFilled(stack) && PermissionUtil.hasPermission(player, SecurityPermissions.EXTRACT, (IPart) this.getTerminal()))
			{
			}
			else
			{

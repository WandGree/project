...
public class ItemStoragePhysical extends ItemECBase implements IStorageCell, IAEItemPowerStorage, IEnergyContainerItem
{
	...
	public int extractEnergy(ItemStack container, int maxExtract, boolean simulate)
	{
		...
		if (simulate)
			...
			return (int) PowerUnits.AE.convertTo(PowerUnits.RF, this.extractAEPower(container, PowerUnits.RF.convertTo(PowerUnits.AE, maxExtract)));
	}
	...
	public double getDurabilityForDisplay(ItemStack itemStack)
	{
		...
		return 1 - this.getAECurrentPower(itemStack) / this.MAX_POWER;
	}
	...
	public AccessRestriction getPowerFlow(ItemStack itemStack)
	{
		if (itemStack == null)
			return null;
		return itemStack.getItemDamage() == 4 ? AccessRestriction.READ_WRITE : AccessRestriction.NO_ACCESS;
	}
	...
	private static boolean cantReplace(EntityPlayer player, int x, int y, int z)
	{
		return EventUtils.cantBreak(player, x, y, z) || player.worldObj.getTileEntity(x, y, z) instanceof IInventory;
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float xOffset, float yOffset, float zOffset)
	{
		...
		{
			double power = this.getAECurrentPower(stack);
			...
			if (world.getBlock(x + face.offsetX, y + face.offsetY, z + face.offsetZ) == Blocks.air && storageStack.getStackSize() != 0 && power >= 20.0D)
				if (!world.isRemote)
				{
					...
					if (block.getItem() instanceof ItemBlock)
					{
						...
						{
							// TODO gamerforEA code start
							if (EventConfig.inList(EventConfig.storagePhysicalBlackList, block))
								return false;
							...
							switch (stack.getTagCompound().getInteger("mode"))
							{
								case 0:
									...
									AEApi.instance().registries().cell().getCellInventory(stack, null, StorageChannel.ITEMS).extractItems(request, Actionable.MODULATE, new PlayerSource(player, null));
									this.extractAEPower(player.getCurrentEquippedItem(), 20.0D);
									break;
								case 1:
									// TODO gamerforEA code start
								...
								case 2:
									...
									if (storageStack.getStackSize() > 9 && power >= 180.0D)
											...
											case DOWN:
												for (int posX = x - 1; posX < x + 2; posX++)
												{
													for (int posZ = z - 1; posZ < z + 2; posZ++)
													{
														...
														{
															// TODO gamerforEA code start
															if (cantReplace(player, posX, y, posZ))
																continue;
															// TODO gamerforEA code end
											...
											case EAST:
												for (int posZ = z - 1; posZ < z + 2; posZ++)
												{
													for (int posY = y - 1; posY < y + 2; posY++)
													{
														...
														{
															// TODO gamerforEA code start
															if (cantReplace(player, x, posY, posZ))
																continue;
															// TODO gamerforEA code end
											...
											case NORTH:
												for (int posX = x - 1; posX < x + 2; posX++)
												{
													for (int posY = y - 1; posY < y + 2; posY++)
													{
														...
														{
															// TODO gamerforEA code start
															if (cantReplace(player, posX, posY, z))
																continue;
															// TODO gamerforEA code end
					...
					{
						player.addChatMessage(new ChatComponentTranslation("extracells.tooltip.onlyblocks"));
						return false;
					}
				}
				else
	...
	public int receiveEnergy(ItemStack container, int maxReceive, boolean simulate)
	{
		...
		{
			int notStored = (int) PowerUnits.AE.convertTo(PowerUnits.RF, this.injectAEPower(container, PowerUnits.RF.convertTo(PowerUnits.AE, maxReceive)));
